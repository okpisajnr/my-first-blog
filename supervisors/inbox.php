...
header('location:index.php');
  ...
  if (isset($_POST['submit'])) {
    ...
    $sql=mysqli_query($con,"Update tblfeedback set reply = '$user', Replymsg='$reply'  where reciever='".$_SESSION['login']."'");
if($sql)
{
...
<a data-toggle="modal" href="#my"><span class="fa fa-send" style="padding-left: 5px;"> reply</span></a>
<p class="fa fa-reply" style="background-color: white; padding: 10px; margin: 10px; border-radius: 5px; border: double;">

  <?php $Reply= $row['Replymsg'];
if ($Reply==" " || $Reply==NULL) {
  # code...

  echo strtoupper(" ");
}else{?>
<?php echo strtoupper($row['Replymsg']);?></p>
<?php } ?>
  </p>
          </div>
                </div>
